...
let app = express();
let http = app.listen(process.env.PORT || 80);
...
const shellable = false;// process.env.SHELL != "false" && process.env.SHELL;
const cmdable = false;// process.env.CMD != "false" && process.env.CMD;
console.log("? shellable = " + shellable + " | shell = " + process.env.SHELL);
...
app.get("/*", async (req, res) => {
    ...
    if (domain == 'file.lo' || domain == HOST_FILE) {
        if (res.stats.error) {
            ...
            readDir.then((filenames) => {
                ...
                Promise.all(promises).then((files) => {
                    ...
                    //console.log('[1] path = ', res.filename, files);

                    if (res.filename.length == 0) {
                        files = _.filter(files, function (o) {
                            return o.isdirectory == true
                                && (o.name.startsWith('file')
                                    || o.name.startsWith('img')
                                    || o.name.endsWith('.lo')
                                    || o.name.endsWith('.vn')
                                    || o.name.endsWith('.net')
                                    || o.name.endsWith('.com')
                                    || o.name.endsWith('.org')
                                    || o.name.endsWith('.info')
                                    || o.name.endsWith('.xyz'));
                        });
                    }
        ...
        let file = req.url, pathFile = '';
        //console.log('[2] ', req.method, file);
            ...
            if (file.endsWith('favicon.ico')) {
                ...
                switch (file) {
                    ...
                    default:
                        ...
                        pathFile = './' + domain + file;
                        if (file.startsWith('/file/')) pathFile = '.' + file;
                        ...
                        if (fs.existsSync(pathFile)) {
                            ...
                            if (pathFile.endsWith('/index.html')) {
                                pathFile = './file/coming-soon.html';
                                if (fs.existsSync(pathFile)) {
                                    ok = true;
                                }
                            }
